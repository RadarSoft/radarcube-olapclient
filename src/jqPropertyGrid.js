	...
	$.fn.jqPropertyGrid = function(obj, meta) {
		...
		for (var prop in obj) {
			...
			if (currGroup !== OTHER_GROUP_NAME && !groupsHeaderRowHTML[currGroup]) {
			    groupsHeaderRowHTML[currGroup] = getGroupHeaderRowHtml(currGroup, meta[prop].groupclass);
			}
		...
		var innerHTML = '<div class="ui-widget pg-buttons" style="display:none">' +
                            '<div class="pg-button pg-button-expandall ui-state-default ui-corner-all" title="Expand all">' +
                                '<span class="ui-icon ui-icon-plusthick"></span>'  +
                            '</div>' +
                            '<div class="pg-button pg-button-collapseall ui-state-default ui-corner-all" title="Collapse all">' +
                                '<span class="ui-icon ui-icon-minusthick"></span>' +
                            '</div>' +
                        '</div>';
		innerHTML += '<div style="max-height: 400px; overflow-y: scroll; clear:both"><table class="pgTable ui-widget ui-widget-content">';
...
//		    innerHTML += getGroupHeaderRowHtml(OTHER_GROUP_NAME, meta[prop].groupclass);
		...
		$(this).find(".pg-button").hover(function () {
		...
		$(this).find('.pg-button-expandall').click({ pg: this }, function (event) {
		    event.data.pg.find('.pgTable').treegrid('expandAll');
		});
		$(this).find('.pg-button-collapseall').click({ pg: this }, function (event) {
		    event.data.pg.find('.pgTable').treegrid('collapseAll');
		});
	 ...
	 */
	function getGroupHeaderRowHtml(displayName, classes) {
	    return '<tr class="ui-widget ui-widget-content ui-state-default ' + classes + '"><td colspan="2" class="pgGroupCell">' + displayName + '</td></tr>';
	}
	...
	function getPropertyRowHtml(pgId, name, value, meta, postCreateInitFuncs, getValueFuncs) {
		...
		if (type === 'boolean' || (type === '' && typeof value === 'boolean')) {
		    valueHTML = '<input class="rs-pg-editor" type="checkbox" id="' + elemId + '" value="' + name + '"' + (value ? ' checked' : '') + ' />';
			if (getValueFuncs) {
		    ...
		    valueHTML = '<input class="rs-pg-editor" type="text" id="' + elemId + '" value="' + value + '" style="width:50px" />';
		    ...
		    valueHTML = '<input class="rs-pg-editor" type="text" id="' + elemId + '" />';
			if (postCreateInitFuncs) {
			...
			if (typeof meta.description === 'string' && meta.description) {
				valueHTML = '<label id="' + elemId + '" title="' + meta.description + '">' + value + '</label>';
			} else {
				valueHTML = '<label id="' + elemId + '">' + value + '</label>';
			}
			if (getValueFuncs) {
			    getValueFuncs[name] = function () {
			        return $('#' + elemId).html();
		    ...
		    valueHTML = '<input class="ui-widget ui-widget-content" type="text" id="' + elemId + '" value="' + value + '"</input>';
			if (getValueFuncs) {
				getValueFuncs[name] = function() {
					return $('#' + elemId).val();
				};
		...
		if (typeof meta.description === 'string' && meta.description &&
		    ...
		    valueHTML += '<span class="pgTooltip" title="' + meta.description + '">[?]</span>';
		}

		return '<tr class="pgRow ' + meta.classes + '"><td class="pgCell ui-widget ui-widget-content"><span>' + displayName + '</span></td><td class="pgCell ui-widget ui-widget-content">' + valueHTML + '</td></tr>';
	}
	...
	function getSelectOptionHtml(id, selectedValue, options) {
		...
		var html = '<select class="rs-pg-editor"';
		if (id) {
			html += ' id="' + id + '"';
		}
